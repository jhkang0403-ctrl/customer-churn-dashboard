...
def load_customers() -> pd.DataFrame:
    df = pd.read_csv(os.path.join(c.DATA_DIR, "data_customers.csv"), encoding="utf-8-sig")
...
def load_voc() -> pd.DataFrame:
    return pd.read_csv(os.path.join(c.DATA_DIR, "data_voc.csv"), encoding="utf-8-sig")


@st.cache_data
def load_consultations() -> pd.DataFrame:
    return pd.read_csv(os.path.join(c.DATA_DIR, "data_consultations.csv"), encoding="utf-8-sig")


@st.cache_data
def load_satisfaction() -> pd.DataFrame:
    return pd.read_csv(os.path.join(c.DATA_DIR, "data_satisfaction.csv"), encoding="utf-8-sig")


@st.cache_data
def load_usage_history() -> pd.DataFrame:
    return pd.read_csv(os.path.join(c.DATA_DIR, "data_usage_history.csv"), encoding="utf-8-sig")
...
def build_voc_chart(customers: pd.DataFrame) -> go.Figure:
    ...
    fig = px.bar(
        ...
        y="이탈율",
        color="구분",
        color_discrete_map={"전체 고객": c.COLOR_NEUTRAL, "해지관련 부정 VOC 이력 있음": c.COLOR_CRITICAL},
...
def build_recontact_bucket_chart(customers: pd.DataFrame) -> go.Figure:
    ...
    fig = px.bar(
        ...
        y="이탈율",
        color="재문의구간",
        color_discrete_map={"0회": c.COLOR_NEUTRAL, "1회": c.COLOR_NEUTRAL, "2회 이상": c.COLOR_CRITICAL},
        text=summary["이탈율"].map(lambda v: f"{v:.1f}%"),
        custom_data=["고객수", "이탈고객수", "이탈율"],
...
def build_plan_chart(customers: pd.DataFrame) -> go.Figure:
    ...
    fig = px.bar(
        ...
        color="plan",
        color_discrete_map={
            plan: (c.COLOR_CRITICAL if plan == highlight_plan else c.COLOR_NEUTRAL) for plan in summary["plan"]
        },
        text=summary["이탈율"].map(lambda v: f"{v:.1f}%"),
        custom_data=["고객수", "이탈고객수", "이탈율"],
...
def build_region_chart(customers: pd.DataFrame) -> go.Figure:
    ...
    fig = px.bar(
        ...
        color="region",
        color_discrete_map={
            region: (c.COLOR_CRITICAL if region in highlight_regions else c.COLOR_NEUTRAL)
...
def build_tenure_usage_scatter(customers: pd.DataFrame) -> go.Figure:
    ...
    df = customers.copy()
    df["tenure_months"] = (c.REFERENCE_DATE.year - df["join_date"].dt.year) * 12 + (
        c.REFERENCE_DATE.month - df["join_date"].dt.month
    )
    ...
    fig = px.scatter(
        ...
        y="avg_data_gb",
        color="이탈여부",
        color_discrete_map={"유지": c.COLOR_NEUTRAL, "이탈": c.COLOR_CRITICAL},
...
def load_enps(team: str) -> tuple[float, int]:
    client = c.get_bigquery_client()
    if client is None:
        raise RuntimeError("BigQuery client unavailable")
    where_clause = "WHERE team = @team" if team != "전체" else ""
    query = f"""
        SELECT agent_satisfaction
        FROM `{c.BQ_PROJECT_ID}.{c.BQ_DATASET}.agents`
        {where_clause}
    """
    df = client.query(query, job_config=c.team_job_config(team)).to_dataframe()
...
def load_burnout_csat(team: str) -> pd.DataFrame:
    ...
    query = f"""
        ...
        FROM `{c.BQ_PROJECT_ID}.{c.BQ_DATASET}.agents` a
        JOIN `{c.BQ_PROJECT_ID}.{c.BQ_DATASET}.consultations` c ON c.agent_id = a.agent_id
        JOIN `{c.BQ_PROJECT_ID}.{c.BQ_DATASET}.satisfaction` s ON s.consult_id = c.consult_id
        {where_clause}
        GROUP BY a.agent_id, a.overtime_hours_avg
    """
    return client.query(query, job_config=c.team_job_config(team)).to_dataframe()
...
def build_burnout_csat_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig = px.scatter(
        ...
        trendline_color_override=c.COLOR_LINE,
        custom_data=["agent_id", "overtime_hours_avg", "csat_avg"],
        labels={"overtime_hours_avg": "월 평균 초과근무시간", "csat_avg": "CSAT 평균"},
    )
    fig.update_traces(
        selector=dict(mode="markers"),
        marker=dict(size=12, color=c.COLOR_BAR, opacity=0.85, line=dict(width=1, color="white")),
        hovertemplate=(
...
def load_training_comparison(team: str) -> pd.DataFrame:
    ...
    query = f"""
        ...
        with_csat AS (
            SELECT b.*, s.csat
            FROM base b
            JOIN `{c.BQ_PROJECT_ID}.{c.BQ_DATASET}.satisfaction` s ON s.consult_id = b.consult_id
        )
        SELECT
...
def _agent_snapshot_date_label() -> str:
    ...
    mtime = datetime.fromtimestamp(os.path.getmtime(os.path.join(c.DATA_DIR, "agents_snapshot.csv")))
    return f"{mtime.month}월 {mtime.day}일"


def _load_agents_snapshot() -> pd.DataFrame:
    return pd.read_csv(os.path.join(c.DATA_DIR, "agents_snapshot.csv"), encoding="utf-8-sig")


def _load_agent_consultations_snapshot() -> pd.DataFrame:
    return pd.read_csv(os.path.join(c.DATA_DIR, "agent_consultations_snapshot.csv"), encoding="utf-8-sig")
...
def build_training_comparison_chart(df: pd.DataFrame) -> go.Figure:
    group_order = ["N", "Y"]
    colors = [c.COLOR_NEUTRAL, c.COLOR_BAR]
...
st.plotly_chart(build_voc_chart(customers_df), width="stretch")

st.subheader("② 채널·만족도로 본 이탈")
st.plotly_chart(build_channel_csat_chart(), width="stretch")

st.subheader("③ 재문의 반복으로 본 이탈")
st.plotly_chart(build_recontact_bucket_chart(customers_df), width="stretch")

st.subheader("④ 요금제로 본 이탈")
st.plotly_chart(build_plan_chart(customers_df), width="stretch")

st.subheader("⑤ 지역으로 본 이탈")
st.plotly_chart(build_region_chart(customers_df), width="stretch")

st.subheader("⑥ 가입기간·이용량으로 본 이탈")
st.plotly_chart(build_tenure_usage_scatter(customers_df), width="stretch")
...
selected_team = st.selectbox("팀 선택", c.TEAM_OPTIONS)

(enps_value, enps_n), enps_source = c.load_with_fallback(load_enps, load_enps_snapshot, selected_team)
burnout_df, burnout_source = c.load_with_fallback(load_burnout_csat, load_burnout_csat_snapshot, selected_team)
training_df, training_source = c.load_with_fallback(
